...
- 활성 사용자 60명이지만 **78%가 (direct)/(none)** — 대부분 launch 디렉토리(NewTool.site/KittyLaunch/Twelve Tools 등) 경유로 "신제품 구경"하러 온 트래픽이지 "자재 계산하러 검색해서 온" 구매의도 트래픽이 아님. google/organic은 4명뿐.
...
3. **google/organic 유입 비중**이 지금 4명/60명(6.7%) → 절대 인원 기준 최소 수십 명 단위로 증가 (검색 유입 = 구매의도 있는 방문자)
